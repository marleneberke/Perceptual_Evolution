...
function is_veridical(player)
    ...
    i = 2
    repeats = false #is same color repeated?
    while !repeats  && i <= length(player)
        if player[i] !== color_order[length(color_order)]
            if sum(color_order.==player[i]) > 0 #if that color appeared earlier
                repeats = true
            end
...
#world (the set) is
#Implemented for any number of colors
function invertability(player)
    set = collect(1:set_size)
    n_colors = length(colors)
    m = sum(set)/set_size #expectation
    guesses = Array{Float64}(undef, n_colors)
    total = 0
    for c = 1:n_colors
        color = colors[c]
