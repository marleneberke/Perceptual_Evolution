...
function simulate(current_players, file)
    ...
    for generation = 1:n_generations
        fitness_payoffs = ones(n_players) #tracks average fitness payoffs from each game
        fitness_payoffs.*(-1 * penalty_value * n_games)
        for game = 1:n_games
        ...
        # println("max fitness ", maximum(fitness_payoffs))
        # println("min fitness ", minimum(fitness_payoffs))
        #reproduce
...
function check()
    ...
    n = 10000
    for i = 1:n
        avg_utility = avg_utility + sample_utility_function_non_monotonic()[1]
    end
